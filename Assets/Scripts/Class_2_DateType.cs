...
//三條斜線是摘要 (XML 語言) 用來說簡短說明,可加在類別變數上方
/// <summary>
/// 認識資料類型
/// </summary>
...
{
    //變數語法:
    //修飾詞 資料型態 變數名稱 指定 預設值;
    //四大常用資料類型
    //整 數:儲存沒有小數點的數值 int 
    //浮點數:儲存有小數點的數值 float
    //字 串:儲存文字,必須用雙引號 string
    //布林值:儲存正與反 bool 只有兩種值 true false
    public int count = 7;
    public float moveSpeed = 3.5f; //浮點數必須添加f後綴
    public float turnSpeed = 30.7F; //大小寫 F 皆可
    public string character = "蓋倫";
    public bool isDead = false; //是否死亡:否
    public bool gameOver = true; //是否結束:是

    // 整數資料類型
    public byte lv = 16; // byte型別,範圍0~255,大小 8 byte
    public uint coin = 9999; // uint型別,範圍0~4,大小 32 byte
    public long item = 3000; // long 型別,範圍0~92,大小 64 byte
